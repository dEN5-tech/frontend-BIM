...
const ChatRoute = () => {
    ...
    (searchTerm: string) => {
      if (searchTerm) {
        ...
        if (filtered.length === 0) {
          axiosInstance
            .get(`/user/search?nameIncludes=${encodeURIComponent(searchTerm)}`)
            .then((response) => {
  ...
  useEffect(() => {
    axiosInstance.get("/user/search?nameIncludes=").then((response) => {
  ...
  return (
    <>
      <Flex h="100vh">
        ...
        <ChatContainer selectedUser={selectedUser} onOpen={onOpen} />
      </Flex>
